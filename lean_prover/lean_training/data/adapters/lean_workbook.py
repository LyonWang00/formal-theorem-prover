...
def normalize_lean_workbook_record(
    record: Mapping[str, Any],
    index: int,
    *,
    source_name: str = LEAN_WORKBOOK_DATASET,
):
    """Route one ordinary Lean-Workbook row through the shared SFT normalizer."""
    ...
    if not record.get("formal_statement") or not record.get("tactic"):
        if str(record.get("pantograph_verified") or "").lower() not in {
            "success",
            "true",
        }:
            raise ValueError("Lean-Workbook SFT row is not Pantograph verified")
        ...
        return replace(normalized, pantograph_verified=True)
