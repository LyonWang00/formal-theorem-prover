        ...
        *,
        ...
        self.backend = PantographBackend(
            PantographOptions(
                ...
                timeout=startup_timeout or timeout,
                core_options=tuple(core_options),
                server_options=server_options,
                lean_path=lean_path or _explicit_workspace_lean_path(self.project_path),
            )
        )
...
def _explicit_workspace_lean_path(project_path: Path) -> str | None:
    """Use existing local build artifacts without asking Lake to resolve packages.

    This is intentionally conservative: once a local Mathlib package is
    present, a missing umbrella OLean is treated as an incomplete workspace
    and must not fall back to Lake.  The project build path comes first so a
    local module cannot be shadowed by a dependency with the same name.
    """

    packages = project_path / ".lake" / "packages"
    mathlib_lib = packages / "mathlib" / ".lake" / "build" / "lib" / "lean"
    mathlib_package = packages / "mathlib"
    manifest_declares_mathlib = False
    manifest = project_path / "lake-manifest.json"
    if manifest.is_file():
        try:
            payload = json.loads(manifest.read_text(encoding="utf-8-sig"))
            manifest_declares_mathlib = any(
                package.get("name") == "mathlib"
                for package in payload.get("packages", [])
                if isinstance(package, dict)
            )
        except (OSError, json.JSONDecodeError, AttributeError) as error:
            raise RuntimeError(
                f"cannot validate local Mathlib from project manifest: {manifest}; "
                "refusing destructive Lake fallback"
            ) from error
    if manifest_declares_mathlib and not mathlib_package.is_dir():
        raise FileNotFoundError(
            f"project declares local Mathlib but its package directory is missing: "
            f"{mathlib_package}; refusing destructive Lake fallback"
        )
    if mathlib_package.is_dir() and not (mathlib_lib / "Mathlib.olean").is_file():
        raise FileNotFoundError(
            f"local Mathlib package exists but its umbrella OLean is missing: "
            f"{mathlib_lib / 'Mathlib.olean'}; refusing destructive Lake fallback"
        )
    if not (mathlib_lib / "Mathlib.olean").is_file():
        return None
    root_lib = project_path / ".lake" / "build" / "lib" / "lean"
    paths = [root_lib] if root_lib.is_dir() else []
    paths.append(mathlib_lib)
    paths.extend(
        sorted(
            path
            for path in packages.glob("*/.lake/build/lib/lean")
            if path != mathlib_lib and path.is_dir()
        )
    )
...
class PantographTaskVerifier:
    ...
    def verify(self, task: VerificationTask) -> dict[str, Any]:
        ...
        if task.payload and task.payload.get("preassembled_source"):
            # Current-commit LeanDojo traces preserve the exact source prefix,
            # including its original import commands.  Wrapping that source in
            # an evaluation namespace would make the imports syntactically
            # invalid and would no longer be source-faithful.
            code = task.lean_code
        else:
