...
class VerificationPool:
    ...
    def start(self) -> None:
        ...
        # Spawn every worker before waiting.  Pantograph/mathlib warmup is
        # independent per process, so serial startup wastes one full server
        # startup interval for every additional worker.
    ...
    def _restart_worker(self, worker_id: int, reason: str) -> None:
        ...
        if self.worker_restart_counts[worker_id] > self.config.max_worker_restarts:
            self.fatal_errors.append(
