#!/usr/bin/env python3
"""Train one independently initialized WB/LD budget-support arm."""
...
from scripts.prepare_wb_ld_replay_ablation import REPLAY_ARMS
...
def main() -> None:
    ...
    all_arms = {**ARMS, **REPLAY_ARMS}
    parser.add_argument("--arm", choices=tuple(all_arms), required=True)
    parser.add_argument(
        "--root",
        type=Path,
    ...
    source_manifest = root / all_arms[name]
    ...
    gate_path = root / "audit" / f"eos_gate_{name}.json"
    ...
    if (
        ...
        or gate.get("total_records") != row_count
        or gate.get("records_with_supervised_eos") != row_count
        or gate.get("records_whose_last_valid_label_is_eos") != row_count
    ...
    if len(set(ids)) != row_count or len(set(groups)) != row_count:
    ...
    if trainer.state.global_step != expected_steps:
        raise RuntimeError(
            f"unexpected optimizer steps: {trainer.state.global_step} != {expected_steps}"
        )
